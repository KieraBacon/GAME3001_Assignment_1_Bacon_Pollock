...
bool CollisionManager::lineAABBCheck(ship* object1, GameObject* object2, float angle, float distance)
{
	glm::vec2 lineStart = object1->getPosition();
	glm::vec2 lineEnd = angle == 0.0f? lineEnd = object1->getPosition() + object1->getCurrentDirection() * distance :
		lineEnd = object1->getPosition() + Util::rotateDirectionVec(object1->getCurrentDirection(), angle) * distance;
	// aabb
