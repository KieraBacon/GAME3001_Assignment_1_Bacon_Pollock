...
glm::vec2 Util::rotateDirectionVec(glm::vec2 vec, float angle)
{
	angle *= Util::Deg2Rad;
	float c = cos(angle);
	float s = sin(angle);
...
glm::vec2 Util::convertHeadingToDirection(float heading)
{
	float x = cos(heading * Util::Deg2Rad);
	float y = sin(heading * Util::Deg2Rad);
	glm::vec2 vec = { x, y };
	return vec;
}
